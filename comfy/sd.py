...
def load_bypass_lora_for_models(model, clip, lora, strength_model, strength_clip):
    ...
    logging.debug(
        f"[BypassLoRA] {len(bypass_patches)} bypass adapters, {len(regular_patches)} regular patches"
    )
    ...
    if model is not None:
        ...
        for key, adapter in bypass_patches.items():
            if key in model_sd_keys:
                ...
                logging.warning(
                    f"[BypassLoRA] Adapter key not in model state_dict: {key}"
                )
...
class CLIP:
    ...
    def load_sd(self, sd, full_model=False):
        if full_model:
            return self.cond_stage_model.load_state_dict(
                sd, strict=False, assign=self.patcher.is_dynamic()
            )
        else:
    ...
    def load_model(self, tokens={}):
        ...
        if hasattr(self.cond_stage_model, "memory_estimation_function"):
            memory_used = self.cond_stage_model.memory_estimation_function(
                tokens, device=self.patcher.load_device
            )

        # Set CUDA device context for multi-GPU support
        target_device = self.patcher.load_device
        if target_device.type == "cuda":
            device_id = target_device.index
            original_cuda_device = torch.cuda.current_device()
            if device_id != original_cuda_device:
                torch.cuda.set_device(device_id)
                # Load model with correct device context
...
class VAE:
    def __init__(self, sd=None, device=None, config=None, dtype=None, metadata=None):
        ...
        m, u = self.first_stage_model.load_state_dict(
            sd, strict=False, assign=self.patcher.is_dynamic()
        )
        if len(m) > 0:
...
def load_clip_model_patcher(
    ...
    clip = load_clip(
        ckpt_paths, embedding_directory, clip_type, model_options, disable_dynamic
    )
    return clip.patcher
...
def load_text_encoder_state_dicts(
    ...
    for i in range(len(clip_data)):
        ...
        if "lm_head.weight" in clip_data[i]:
            clip_data[i]["model.lm_head.weight"] = clip_data[i].pop(
                "lm_head.weight"
            )  # prefix missing in some models
    ...
    if len(clip_data) == 1:
        ...
        if te_model == TEModel.CLIP_G:
            ...
            clip_target.clip = comfy.text_encoders.lumina2.te(
                **llama_detect(clip_data), model_type="gemma3_4b_vision"
            )
            clip_target.tokenizer = comfy.text_encoders.lumina2.NTokenizer
            tokenizer_data["spiece_model"] = clip_data[0].get("spiece_model", None)
        elif te_model == TEModel.GEMMA_3_12B:
            clip_target.clip = comfy.text_encoders.lt.gemma3_te(
            ...
            if clip_type == CLIPType.HUNYUAN_IMAGE:
                ...
                clip_target.clip = comfy.text_encoders.longcat_image.te(
                    **llama_detect(clip_data)
                )
                clip_target.tokenizer = (
            ...
            if clip_type == CLIPType.FLUX or clip_type == CLIPType.FLUX2:
                clip_target.clip = comfy.text_encoders.flux.klein_te(
                    **llama_detect(clip_data), model_type="qwen3_4b"
                )
                clip_target.tokenizer = comfy.text_encoders.flux.KleinTokenizer
            else:
                clip_target.clip = comfy.text_encoders.z_image.te(
                    **llama_detect(clip_data)
                )
            ...
            clip_target.clip = comfy.text_encoders.flux.klein_te(
                **llama_detect(clip_data), model_type="qwen3_8b"
            )
        ...
        if clip_type == CLIPType.SD3:
            ...
            clip_target.clip = comfy.text_encoders.lt.ltxav_te(
                **llama_detect(clip_data), **comfy.text_encoders.lt.sd_detect(clip_data)
            )
            ...
            if TEModel.QWEN3_4B in te_models:
                ...
                model_type = "qwen3_2b"
            clip_target.clip = comfy.text_encoders.ace15.te(
                lm_model=model_type, **llama_detect(clip_data)
            )
...
def load_checkpoint_guess_config(
    ...
    if output_model and out[0] is not None:
        out[0].cached_patcher_init = (
            load_checkpoint_guess_config_model_only,
...
def load_checkpoint_guess_config_model_only(
    ckpt_path,
    embedding_directory=None,
    model_options={},
    te_model_options={},
    ...
    model, *_ = load_checkpoint_guess_config(
        ckpt_path,
        False,
        False,
        False,
...
def load_checkpoint_guess_config_clip_only(
    ckpt_path,
    embedding_directory=None,
    model_options={},
    te_model_options={},
    ...
    _, clip, *_ = load_checkpoint_guess_config(
        ckpt_path,
        False,
        True,
        False,
...
def load_state_dict_guess_config(
    ...
    if output_model:
        ...
        model.load_model_weights(
            sd, diffusion_model_prefix, assign=model_patcher.is_dynamic()
        )
...
def load_diffusion_model_state_dict(
    sd, model_options={}, metadata=None, disable_dynamic=False
):
    """
